...
class ConnecticutScraper(RequestsScraper):
    ...
    def _fetch_page(self, offset, customerid):
        ...
        try:
            ...
            raise SearchTimeoutError(f"Connecticut HTTP request failed at offset {offset}") from re

        content_type = resp.headers.get("Content-Type", "")
        if "application/json" not in content_type.lower():
            snippet = resp.text[:200].replace("\n", " ")
            self.logger.error(f"Expected JSON but got: {snippet!r}")
            raise DataExtractionError("Connecticut expected JSON but received non‑JSON content")
        ...
        try:
            ...
            raise DataExtractionError(f"Connecticut JSON parsing failed at offset {offset}") from e
    ...
    def search(self, **kwargs):
        ...
        try:
            ...
            raise ScraperError("Connecticut search failed") from e
    ...
    def extract_data(self, page_content):
        if not page_content or "records" not in page_content:
            ...
            raise DataExtractionError("Connecticut extract_data missing records")
        ...
        try:
            ...
            raise DataExtractionError("Connecticut extract_data failed") from e
    ...
    def scrape(self, **kwargs):
        ...
        try:
            ...
            raise ScraperError("Connecticut scrape failed") from e